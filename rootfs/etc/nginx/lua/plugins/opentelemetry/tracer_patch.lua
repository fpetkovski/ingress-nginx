--------------------------------------------------------------------------------
-- This tracer monkey-patches opentelemetry-lua's tracer, but updates it to
-- propagate span context onto nonrecording spans. This preserves span parentage
-- when the verbosity sampler marks spans as nonrecording.
--------------------------------------------------------------------------------
...
local function new_span(self, context, name, config)
    ...
    if not sampling_result:is_recording() then
        new_span_context.span_id = span_context and span_context.span_id or '0000000000000000'
        new_span_context.trace_id = span_context and span_context.trace_id or '00000000000000000000000000000000'
...
--                      span
--------------------------------------------------------------------------------
function tracer.start(self, context, span_name, span_start_config)
