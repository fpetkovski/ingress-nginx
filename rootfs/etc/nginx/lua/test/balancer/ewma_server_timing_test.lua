...
local function flush_all_ewma_stats()
  ngx.shared.balancer_ewma_server_timing:flush_all()
  ngx.shared.balancer_ewma_server_timing_last_touched_at:flush_all()
end

local function store_ewma_stats(endpoint_string, ewma, touched_at)
  ngx.shared.balancer_ewma_server_timing:set(endpoint_string, ewma)
  ngx.shared.balancer_ewma_server_timing_last_touched_at:set(endpoint_string, touched_at)
end

local function assert_ewma_stats(endpoint_string, ewma, touched_at)
  assert.are.equals(ewma, ngx.shared.balancer_ewma_server_timing:get(endpoint_string))
  assert.are.equals(touched_at, ngx.shared.balancer_ewma_server_timing_last_touched_at:get(endpoint_string))
end


describe("Balancer ewma", function()
  local balancer_ewma_server_timing = require("balancer.ewma_server_timing")
  local ngx_now = 1543238266
  local backend, instance

  before_each(function()
    mock_ngx({ now = function() return ngx_now end, var = { balancer_ewma_server_timing_score = -1 } })
    package.loaded["balancer.ewma_server_timing"] = nil
    balancer_ewma_server_timing = require("balancer.ewma_server_timing")
    ...
    instance = balancer_ewma_server_timing:new(backend)
  end)
  ...
  describe("after_balance()", function()
    ...
    it("sets state value from Server-Timing header when util field exists", function()
      ...
      assert.are.equals(0.5, ngx.shared.balancer_ewma_server_timing:get(ngx.var.upstream_addr))
    end)
    ...
    it("overrides previous state value with new value from Server-Timing header", function()
      ...
      assert.are.equals(0.6, ngx.shared.balancer_ewma_server_timing:get(ngx.var.upstream_addr))
    end)
  ...
  describe("balance()", function()
    it("returns single endpoint when the given backend has only one endpoint", function()
      ...
      local single_endpoint_instance = balancer_ewma_server_timing:new(single_endpoint_backend)

      local peer = single_endpoint_instance:balance()

      assert.are.equals("10.10.10.1:8080", peer)
      assert.are.equals(-1, ngx.var.balancer_ewma_server_timing_score)
    end)
    ...
    it("doesn't pick the tried endpoint while retry", function()
      ...
      local two_endpoints_instance = balancer_ewma_server_timing:new(two_endpoints_backend)

      ngx.ctx.balancer_ewma_server_timing_tried_endpoints = {
        ["10.10.10.3:8080"] = true,
      }
      local peer = two_endpoints_instance:balance()
      assert.equal("10.10.10.1:8080", peer)
      assert.equal(true, ngx.ctx.balancer_ewma_server_timing_tried_endpoints["10.10.10.1:8080"])
    end)
